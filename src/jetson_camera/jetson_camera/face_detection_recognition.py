...
class FaceRecognitionNode(Node):
    def __init__(self):
        ...
        self.max_scan_duration = 2.0 # Seconds
        ...
        if not self.is_scanning:
            ...
            if self.recognised:
                self.state_history.append({
                    "identity": self.remembered_identity,
                    "angle": target_angle
                })
            ...
            if elapsed > self.max_scan_duration:
                self.get_logger().info("Scan timed out - no face detected.")
    ...
    def evaluate_and_publish_final_result(self):
        ...
        if not self.state_history:
            output = {"status": "complete", "identity": "Unknown", "final_angle": 0.0}
        else:
            angles = [item["angle"] for item in self.state_history]
            avg_angle = sum(angles) / len(angles)
            output = {
                "status": "complete", 
                "identity": self.remembered_identity,
                "final_angle": round(avg_angle, 4)
            }
        
        self.result_pub.publish(String(data=json.dumps(output)))
        self.get_logger().info(f"Scan complete: {output}")
