...
# IMPORTANT: Change 'robot_controller' to whatever you named the file 
# containing your PositionController class. 
...
class FaceTrackingSubscriber(Node):
    def __init__(self):
        super().__init__('face_tracking_subscriber')
        
        self.get_logger().info("Initializing Face Tracking Subscriber...")

        # Initialize your custom robot controller
        self.controller = PositionController(trim=0.0)
        
        # Subscribe to the rotation angle published by the camera node
        self.subscription = self.create_subscription(
            Float32,
            '/face_tracking/rotation_angle',
            self.angle_callback,
            10  # Queue size
        )
        ...
        # Deadband (in radians). If the face is within this angle, don't move.
        # 0.08 radians is roughly 4.5 degrees. This prevents constant jittering.
        self.deadband_rad = 0.08 
        
        # The speed at which the robot will rotate
        self.turn_velocity = 0.3 
        
        # State flag to prevent overlapping commands
        self.is_moving = False

        self.get_logger().info("Subscriber is ready and listening!")

    def angle_callback(self, msg):
        # 1. Ignore new messages if the robot is currently rotating
        if self.is_moving:
            return

        target_angle = msg.data

        # 2. Check the deadband (Is the face already centered enough?)
        if abs(target_angle) < self.deadband_rad:
            # Face is centered, do nothing
            return
        ...
        try:
            ...
            self.get_logger().info(
                f"Turning {direction} by {abs(target_angle):.3f} rad..."
            )
            
            # Call your blocking rotate function
            self.controller.rotate(angle=-target_angle, velocity=self.turn_velocity)
            
            self.get_logger().info("Turn complete. Searching for face...")
            ...
            # Allow the next camera frame to trigger a movement
            self.is_moving = False

def main(args=None):
    rclpy.init(args=args)
    
    node = FaceTrackingSubscriber()
    
    try:
        # Spin keeps the node alive to listen for callbacks
        rclpy.spin(node)
    except KeyboardInterrupt:
        node.get_logger().info("Keyboard Interrupt detected. Shutting down...")
    finally:
        # Safely shut down the motors and clean up
